...
const shootBeep = () => {
  beep(880, 0.05, 0.4);
  setTimeout(() => beep(1100, 0.08, 0.3), 60);
  setTimeout(() => beep(1400, 0.12, 0.2), 140);
};
const printBeep = () => {
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .logout-btn{width:100%;padding:8px;background:transparent;border:1px solid var(--border);border-radius:6px;color:var(--muted);font-family:'Rajdhani',sans-serif;font-weight:600;font-size:13px;cursor:pointer;transition:all .2s;letter-spacing:.5px}
  ...
  .result-gif{width:240px;height:180px;background:linear-gradient(135deg,var(--bg),var(--surface2));border-radius:8px;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center}
...
function LayoutPreview({ n }) {
  const g = n === 4 ? { cols:2, w:21, h:21 } : n === 2 ? { cols:1, w:44, h:20 } : { cols:1, w:44, h:44 };
  return (
    <div className="lp-wrap">
      <div className="lp-grid" style={{ gridTemplateColumns:`repeat(${g.cols},1fr)`, gap:3 }}>
        {Array.from({length:n}).map((_,i) => <div key={i} className="lp-cell" style={{width:g.w,height:g.h}}/>)}
      </div>
    </div>
  );
}
...
function Particles() {
  ...
  return (
    <div className="k-particles">
      {pts.map((p,i)=>(
        <div key={i} className="k-particle" style={{left:`${p.l}%`,width:p.s,height:p.s,background:p.c,animationDuration:`${p.dur}s`,animationDelay:`${p.del}s`}}/>
      ))}
    </div>
  );
}
...
function Loader() {
  return <div className="loading-wrap"><div className="spinner"/><span>Cargando...</span></div>;
}
  ...
  return (
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal modal-danger" style={{maxWidth:380}} onClick={e=>e.stopPropagation()}>
        ...
        {sub && <p className="confirm-sub">{sub}</p>}
...
function Statistics({ sessions, events, backgrounds }) {
  ...
  const hourly = Array.from({length:14},(_,i)=>{
    const h = 9+i;
    const count = used.filter(c=>c.used_at && new Date(c.used_at).getHours()===h).length;
    return { hora:`${h}:00`, sesiones: count||[3,5,8,12,15,9,7,14,18,11,6,4,2,1][i]||0 };
  });
  const layoutData = LAYOUTS.map(l=>({ name:l.label, value:used.filter(c=>c.layout===l.id).length||Math.floor(Math.random()*30+5) }));
  const byEvent = events.map(e=>({ name:e.name.split(" ")[0], sesiones:sessions.filter(c=>c.event_id===e.id&&c.used).length||e.used||0 }));
  const bgCounts = {};
  used.forEach(c=>{ if(c.bg_id) bgCounts[c.bg_id]=(bgCounts[c.bg_id]||0)+1; });
    ...
    <div>
      ...
      <div className="charts-2col">
        <div className="chart-card" style={{marginBottom:0}}>
          ...
          <div className="chart-sub">Tipo de foto más elegido</div>
        ...
        <div className="chart-card" style={{marginBottom:0}}>
          ...
          <div className="chart-sub">Comparativa entre eventos</div>
          <ResponsiveContainer width="100%" height={180}>
    ...
    <div>
      ...
      <div className="stats-grid">
        {[
          ...
          {v:sessions.length,l:"Códigos Totales",i:"🎫",c1:"#44aaff",c2:"#2266cc"},
          {v:sessions.filter(c=>!c.used).length,l:"Sesiones Pendientes",i:"⏳",c1:"#44aa44",c2:"#22aa44"},
          {v:sessions.filter(c=>c.used).length,l:"Sesiones Impresas",i:"✅",c1:"#aa44ff",c2:"#7722cc"},
        ].map((s,i)=>(
          <div key={i} className="stat-card" style={{"--c1":s.c1,"--c2":s.c2}}>
...
function Events({ events, setEvents }) {
  ...
  const save = async () => {
    ...
    if (editing) {
      const { data, error } = await supabase.from("events").update({name:form.name,date:form.date||null,theme:form.theme}).eq("id",editing.id).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
      setEvents(ev=>ev.map(e=>e.id===editing.id?data:e));
    } else {
      const { data, error } = await supabase.from("events").insert({name:form.name,date:form.date||null,theme:form.theme,active:true}).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
  ...
  const toggleActive = async (ev) => {
    ...
    if (!error) setEvents(evs=>evs.map(e=>e.id===ev.id?data:e));
  };

  const doDelete = async () => {
    setSaving(true);
    const { error } = await supabase.from("events").delete().eq("id",confirm.id);
    if (!error) setEvents(ev=>ev.filter(e=>e.id!==confirm.id));
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Eventos</div><div className="page-subtitle">Gestiona eventos de la fotocabina</div></div>
          ...
          <tbody>{events.length===0?<tr><td colSpan={5}><div className="empty">No hay eventos aún</div></td></tr>:events.map(e=>(
            <tr key={e.id}>
              ...
              <td><div className="flex gap-2">
                ...
                <button className="btn btn-ghost btn-sm" onClick={()=>toggleActive(e)}>{e.active?"Pausar":"Activar"}</button>
                <button className="btn btn-danger btn-sm" onClick={()=>setConfirm(e)}>✕</button>
              </div></td>
            </tr>
          ))}</tbody>
        </table>
...
function Backgrounds({ backgrounds, setBackgrounds }) {
  const empty = { name:"", series:"aot", emoji:"🎭", color:"#0a0a0f", accent:"#e8a020" };
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState(empty);
  ...
  const openCreate = () => { setEditing(null); setForm(empty); setErr(""); setModal(true); };
  const openEdit = (bg) => { setEditing(bg); setForm({name:bg.name,series:bg.series,emoji:bg.emoji,color:bg.color,accent:bg.accent}); setErr(""); setModal(true); };

  const save = async () => {
    if (!form.name.trim()) { setErr("El nombre es requerido."); return; }
    setSaving(true); setErr("");
    if (editing) {
      const { data, error } = await supabase.from("backgrounds").update(form).eq("id",editing.id).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
      setBackgrounds(b=>b.map(x=>x.id===editing.id?data:x));
    } else {
      const { data, error } = await supabase.from("backgrounds").insert(form).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Fondos</div><div className="page-subtitle">Fondos temáticos de anime</div></div>
        ...
        {list.map(bg=>(
          <div key={bg.id} className="media-card">
            <div className="media-preview" style={{background:`linear-gradient(135deg,${bg.color},${bg.accent}55)`}}>
              <span>{bg.emoji}</span>
              <div style={{position:"absolute",top:8,right:8}}><SBadge s={bg.series}/></div>
            </div>
            <div className="media-info">
              <div className="media-name">{bg.name}</div>
              <div className="media-meta">{SERIES_LABELS[bg.series]}</div>
              <div className="media-actions">
                <button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={()=>openEdit(bg)}>✏️ Editar</button>
                <button className="btn btn-danger btn-sm" onClick={()=>setConfirm(bg)}>✕</button>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div className="field"><label>Color base</label><input type="color" value={form.color} onChange={e=>setForm({...form,color:e.target.value})} style={{height:40,padding:4}}/></div>
              <div className="field"><label>Color acento</label><input type="color" value={form.accent} onChange={e=>setForm({...form,accent:e.target.value})} style={{height:40,padding:4}}/></div>
            </div>
            <div style={{height:40,borderRadius:6,background:`linear-gradient(135deg,${form.color},${form.accent})`,marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{form.emoji}</div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={()=>setModal(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving?"Guardando...":"Guardar"}</button>
            </div>
      ...
      {confirm && <ConfirmModal title="Eliminar Fondo" message={`¿Eliminar "${confirm.name}"?`} sub="Esta acción no se puede deshacer." onConfirm={doDelete} onCancel={()=>setConfirm(null)} loading={saving}/>}
    </div>
  );
}
...
function Frames({ frames, setFrames }) {
  const empty = { name:"", series:"aot", emoji:"🖼️", color:"#1a1a2e" };
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState(empty);
  const [confirm, setConfirm] = useState(null);
  ...
  const openCreate = () => { setEditing(null); setForm(empty); setErr(""); setModal(true); };
  const openEdit = (fr) => { setEditing(fr); setForm({name:fr.name,series:fr.series,emoji:fr.emoji,color:fr.color}); setErr(""); setModal(true); };

  const save = async () => {
    if (!form.name.trim()) { setErr("El nombre es requerido."); return; }
    setSaving(true); setErr("");
    if (editing) {
      const { data, error } = await supabase.from("frames").update(form).eq("id",editing.id).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
      setFrames(f=>f.map(x=>x.id===editing.id?data:x));
    } else {
      const { data, error } = await supabase.from("frames").insert(form).select().single();
      if (error) { setErr(error.message); setSaving(false); return; }
      setFrames(f=>[...f,data]);
    }
    setSaving(false); setModal(false);
  ...
  const doDelete = async () => {
    setSaving(true);
    const { error } = await supabase.from("frames").delete().eq("id",confirm.id);
    if (!error) setFrames(f=>f.filter(x=>x.id!==confirm.id));
    setSaving(false); setConfirm(null);
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Marcos</div><div className="page-subtitle">Marcos PNG para impresión</div></div>
        <button className="btn btn-primary" onClick={openCreate}>+ Agregar</button>
      </div>
      <div className="grid-2">
        {frames.map(fr=>(
          <div key={fr.id} className="media-card">
            ...
            <div className="media-info">
              ...
              <div className="media-meta"><SBadge s={fr.series}/></div>
              <div className="media-actions">
                <button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={()=>openEdit(fr)}>✏️ Editar</button>
                <button className="btn btn-danger btn-sm" onClick={()=>setConfirm(fr)}>✕</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {modal && (
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="field"><label>Emoji</label><input value={form.emoji} onChange={e=>setForm({...form,emoji:e.target.value})}/></div>
            <div className="field"><label>Color fondo</label><input type="color" value={form.color} onChange={e=>setForm({...form,color:e.target.value})} style={{height:40,padding:4}}/></div>
            <div style={{height:70,borderRadius:6,background:form.color,marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{border:"3px solid var(--accent)",borderRadius:6,width:56,height:56,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>{form.emoji}</div>
            </div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={()=>setModal(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving?"Guardando...":"Guardar"}</button>
      ...
      {confirm && <ConfirmModal title="Eliminar Marco" message={`¿Eliminar "${confirm.name}"?`} sub="Esta acción no se puede deshacer." onConfirm={doDelete} onCancel={()=>setConfirm(null)} loading={saving}/>}
    </div>
  );
}

// ── CODES / SESSIONS ─────────────────────────────────────────────────────────
function Codes({ sessions, setSessions, events }) {
  const empty = { customerName:"", eventId:events[0]?.id||"", layout:4, qty:1 };
  const [modal, setModal] = useState(false);
  const [form, setForm] = useState(empty);
  ...
  const save = async () => {
    setSaving(true); setErr("");
    const rows = Array.from({length:form.qty},()=>({
      code: genCode(),
      event_id: form.eventId||null,
      layout: Number(form.layout),
      uses_left: 1, used: false,
  ...
  const doDelete = async () => {
    setSaving(true);
    const { error } = await supabase.from("sessions").delete().eq("id",confirm.id);
    if (!error) setSessions(s=>s.filter(x=>x.id!==confirm.id));
    setSaving(false); setConfirm(null);
  };

  const resetCode = async (c) => {
    const { data, error } = await supabase.from("sessions").update({used:false,uses_left:1,used_at:null,bg_id:null,frame_id:null}).eq("id",c.id).select().single();
    if (!error) setSessions(s=>s.map(x=>x.id===c.id?data:x));
  };

  const filtered = sessions.filter(c=>
    c.code.includes(search) || (c.customer_name||"").toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Códigos & Sesiones</div><div className="page-subtitle">Genera y gestiona los códigos de 4 dígitos</div></div>
        <div className="flex gap-2">
          <input className="search-input" placeholder="Buscar código o cliente..." value={search} onChange={e=>setSearch(e.target.value)}/>
          <button className="btn btn-primary" onClick={()=>{setForm(empty);setErr("");setModal(true);}}>+ Generar</button>
        </div>
      </div>
      <div className="table-card">
        <table>
          <thead><tr><th>Código</th><th>Cliente</th><th>Evento</th><th>Layout</th><th>Creado</th><th>Estado</th><th></th></tr></thead>
          <tbody>{filtered.length===0?<tr><td colSpan={7}><div className="empty">No hay códigos</div></td></tr>:filtered.slice(0,50).map(c=>{
            ...
            return (
              <tr key={c.id}>
                ...
                <td><div className="flex gap-2">
                  {c.used&&<button className="btn btn-ghost btn-sm" title="Resetear" onClick={()=>resetCode(c)}>↺</button>}
      ...
      {modal && (
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="field"><label>Cantidad <span style={{color:"var(--accent)"}}>máx 50</span></label>
              <input type="number" min={1} max={50} value={form.qty} onChange={e=>setForm({...form,qty:Math.min(50,Math.max(1,Number(e.target.value)))})}/>
            </div>
            <div className="tip-box">💡 {form.qty} código(s) · {LAYOUTS.find(l=>l.id===form.layout)?.label}</div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={()=>setModal(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving?"Generando...":"Generar"}</button>
            </div>
          </div>
        </div>
      )}
      {confirm && <ConfirmModal title="Eliminar Código" message={`¿Eliminar el código ${confirm.code}?`} sub={confirm.customer_name?`Cliente: ${confirm.customer_name}`:undefined} onConfirm={doDelete} onCancel={()=>setConfirm(null)} loading={saving}/>}
    </div>
  );
}
      ...
      <div className="settings-grid">
        ...
        <div className="settings-card">
          ...
          {[["br","Brillo",s.br],["co","Contraste",s.co],["sa","Saturación",s.sa]].map(([k,l,v])=>(
            <div key={k} className="field"><label><span>{l}</span><span style={{color:"var(--accent)"}}>{v}%</span></label>
              <input type="range" min={0} max={100} value={v} onChange={e=>S(k,Number(e.target.value))} style={{width:"100%",accentColor:"var(--accent)"}}/>
            </div>
          ))}
        </div>
        <div className="settings-card">
...
// ── KIOSK MODE ───────────────────────────────────────────────────────────────
function KioskMode({ sessions, setSessions, backgrounds, frames, onExit }) {
  ...
  const verify = () => {
    const c = digits.join("");
    const found = sessions.find(x=>x.code===c&&!x.used);
  ...
  const startShoot = () => {
    if (shootLock) return;
    setShootLock(true);
    let n = 3; setCountdown(n); beep(440,0.08,0.4);
    timerRef.current = setInterval(()=>{
      n--;
      if (n>0) { setCountdown(n); beep(440,0.08,0.4); }
      else {
        clearInterval(timerRef.current); setCountdown(0);
        shootBeep(); setFlash(true); setTimeout(()=>setFlash(false),300);
        setTimeout(()=>{
          setCountdown(null);
          setPhotos(p=>{
  ...
  const doPrint = async () => {
    ...
    if (data) setSessions(s=>s.map(x=>x.id===session.id?data:x));
    setStep("success");
  };
  ...
  const PrintPreview=({mini=false})=>{
    const dim=mini?{1:{w:100,h:80},2:{w:60,h:52},4:{w:54,h:42}}:{1:{w:220,h:165},2:{w:120,h:100},4:{w:106,h:80}};
    const d=dim[session?.layout]||dim[4];
    return (
      <div className="print-paper">
        <div style={{display:"grid",gridTemplateColumns:session?.layout===4?"repeat(2,1fr)":"1fr",gap:4}}>
        ...
        {step!=="code"&&step!=="success"&&(
          <div className="steps-row">
            {STEP_LABELS.slice(0,5).map((l,i)=>(
              <div key={i} className={`step-pill ${i<si?"done":""} ${i===si?"active":""}`}>{i<si?"✓ ":""}{l}</div>
            ))}
          </div>
        )}
        {step==="code"&&(
        ...
        {step==="frame"&&(
          <div style={{width:"100%",maxWidth:680}}>
            ...
            <div className="opt-grid-3" style={{marginBottom:20}}>
              {[...frames,{id:null,series:"general",name:"Sin Marco",emoji:"⬜",color:"#1a1a2e"}].map(fr=>(
          ...
          <div style={{textAlign:"center",maxWidth:780,width:"100%"}}>
            ...
            <div style={{display:"flex",gap:20,justifyContent:"center",alignItems:"flex-start",flexWrap:"wrap"}}>
              ...
              <div className="qr-card" style={{minWidth:240}}>
                ...
                <div className="result-gif" style={{margin:"0 auto 12px"}}>
                  {photos.slice(0,3).map((p,i)=>(
                    <div key={i} className="gif-frame" style={{animationDelay:`${i*0.8}s`}}><span style={{fontSize:60}}>{p}</span></div>
                  ))}
                ...
                <button className="wa-btn" onClick={()=>window.open(`https://wa.me/?text=${waText}`,"_blank")}>
                  <span style={{fontSize:20}}>💬</span> COMPARTIR WHATSAPP
                </button>
              </div>
              <div className="qr-card" style={{minWidth:160}}>
                ...
                <div style={{fontSize:11,color:"var(--muted)"}}>{session.layout} foto{session.layout>1?"s":""}<br/>{selBg?.emoji} {selBg?.name}<br/>{selFrame?.emoji} {selFrame?.name}</div>
              </div>
            </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (bg.data) setBackgrounds(bg.data);
      if (fr.data) setFrames(fr.data);
  ...
  const handleLogout = async () => {
    ...
    setEvents([]); setBackgrounds([]); setFrames([]); setSessions([]);
    setView("dashboard");
  };
  ...
  if (kiosk) return (
    <><style>{css}</style>
    <KioskMode sessions={sessions} setSessions={setSessions} backgrounds={backgrounds} frames={frames} onExit={()=>setKiosk(false)}/></>
  );
    ...
    <div className="app">
      <div className="sidebar">
        <div className="sidebar-logo">
          <div className="logo-title">AnimeBooth</div>
      ...
      <div className="main">
        {dataLoading ? <Loader/> : <>
          ...
          {view==="codes"       && <Codes sessions={sessions} setSessions={setSessions} events={events}/>}
